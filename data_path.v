...
module latch_type2_extended_for4bit(output reg [3:0]out,input [3:0]in,input [3:0]flush_value,input clk,input stall,input flush);
always@(posedge clk)
begin
    if(flush)
    out<=flush_value;
...
module alu(output reg[31:0]out,input [31:0]A,B,input [3:0]Alu_control);
always@(*)
begin 
case(Alu_control)
0:out=A+B;
1:out=A-B;
2:out=A<<B[4:0];
3:out=($signed(A)<$signed(B));
4:out=A<B;
5:out=A^B;
6:out=A>>B[4:0];
7:out=$signed(A)>>B[4:0];
8:out=A|B;
9:out=A&B;
10:out=A==B;
11:out=A!=B;
12:out=$signed(A)>=$signed(B);
13:out=A>=B;
15:out=B;
default:out=32'dx;
endcase
...
// module memory_exetension_unit(output reg [31:0]out,input [31:0]in,input [1:0]sel,input [2:0]funct3);
// reg [7:0]byte;
// reg [15:0]half_word;
// always@(*)
// begin
...
// begin
//     case(funct3)
//     0:out={24{byte[7]},byte};
//     1:out={16{half_word[15],half_word}};
//     2:out=in;
//     4:out={24'b0,byte};
//     5:out={16'b0,half_word};
//     default:out=in;
//     endcase
// end
// endmodule
module signextent(output reg[31:0]out,input [31:0]in,input[2:0] immsrc);
always@(*)
begin 
case(immsrc)
...
3:out={{12{in[31]}},in[19:12],in[20],in[30:21],1'b0};
4:out={in[31:12],12'b0};
default:out=32'dx;
endcase
end
...
module datapath(output memory_write_enable,register_write_enable,MEM_jump,misprediction,MEM_regwrite,EX_lw,WB_lw,ID_rtype,ID_branch,ID_nc,update_taken,update_en,was_hit,output [31:0]IF_pc,EX_pc,update_target_address,output wire [4:0]A1,A2,A3,MEM_rs2,EX_rs,EX_rt,EX_rd,MEM_rd,WB_rd,output wire [31:0]WD3,WD,A,output [1:0]update_way,input [31:0]Instr,Rd1,Rd2,predicted_address,ReadData,input [2:0]immsrc,input [1:0]sel_a,sel_b,resultsrc,way,input [3:0]Alu_control,input hit,pred_taken,IF_stall,ID_stall,ID_flush,EX_stall,EX_flush,regwrite,write_enable,srcb,selpc,sel_write_data,clk,reset);
parameter lw=7'b0000011,sw=7'b0100011,Rtype=7'b0110011,branch=7'b1100011,ITYPE=7'b0010011,jal=7'b1101111,jalr=7'b1100111,Auipc=7'b0010111,lui=7'b0110111;
...
wire update_en;
wire EX_index,EX_taken,EX_srcb,EX_regwrite,EX_write_enable,EX_jump,EX_branch,EX_lw,EX_jal,EX_jalr;
wire [31:0] MEM_pcplus4,MEM_aluresult,MEM_B,MEM_write_data,MEM_target_address;
wire [1:0]MEM_resultsrc;
wire [31:0]EX_next_address;
wire MEM_regwrite,MEM_write_enable,MEM_jump;
wire [31:0]WB_pcplus4,WB_aluresult,WB_read_data,WB_result,WB_target_address;
...
assign A={MEM_aluresult[31:2],2'b0};
...
//ID section 
wire ID_nc,ID_rtype,ID_branch;
wire [2:0]ID_immsrc;
wire MEM_lw,WB_lw;
wire[3:0] ID_Alu_control;
wire [1:0]ID_resultsrc;
wire ID_srcb,ID_write_enable,ID_regwrite;
assign ID_nc=(ID_IR[6:0]==jal)|(ID_IR[6:0]==jalr)|(ID_IR[6:0]==Auipc)|(ID_IR[6:0]==lui);
assign ID_rtype=ID_IR[6:0]==Rtype;
assign ID_branch=ID_IR[6:0]==branch;
...
latch_type2_extended_for3bit immediate_select(ID_immsrc,immsrc,3'b0,clk,ID_stall,ID_flush);
latch_type2_extended_for4bit alu_control_inid(ID_Alu_control,Alu_control,4'b0,clk,ID_stall,ID_flush);
...
//ex
assign EX_branch=(EX_IR[6:0]==branch);
assign EX_jal=EX_IR[6:0]==jal;
assign EX_jalr=EX_IR[6:0]==jalr;
assign EX_jump=EX_jal|EX_jalr;
assign EX_lw=EX_IR[6:0]==lw;
assign update_taken=EX_jump|(EX_branch&EX_aluresult[0]);
assign update_en=EX_hit|update_taken;
assign misprediction=(update_taken^EX_taken)|(EX_taken&(EX_predicted_address!=update_target_address));
...
latch_type2_extended_for4bit alu_control_inex(EX_alucontrol,ID_Alu_control,4'b0,clk,EX_stall,EX_flush);
...
mux2to1 to_update_target_address_to_btb(update_target_address,EX_target_address,{EX_aluresult[31:1],1'b0},EX_jalr);//still to update
alu arithematic_unit(EX_aluresult,EX_A,EX_B,EX_alucontrol);
mux2to1 to_sel_next_address(EX_next_address,EX_pcplus4,update_target_address,update_taken);
//mem
...
latch_type3 target_address_in_mem(MEM_target_address,EX_target_address,clk);
...
latch_type3 target_address_in_WB(WB_target_address,MEM_target_address,clk);
...
mux4to1 result_selector(WB_result,WB_aluresult,WB_read_data,WB_pcplus4,WB_target_address,WB_resultsrc);
endmodule
